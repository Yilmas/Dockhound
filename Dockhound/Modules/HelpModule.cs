...
namespace Dockhound.Modules
{
    public partial class DockModule
    {
        public class HelpModule : InteractionModuleBase<SocketInteractionContext>
        {
            ...
            [SlashCommand("help", "Retrieve link to documentation")]
            [CommandContextType(InteractionContextType.Guild, InteractionContextType.PrivateChannel)]
            public async Task Help()
            {
                const string docUrl = "https://yilmas.github.io/Dockhound/";

                // Defer so the interaction doesn't time out while building the response
                await DeferAsync(ephemeral: true);

                var eb = new EmbedBuilder()
                    .WithTitle("Dockhound Documentation")
                    .WithUrl(docUrl)
                    .WithDescription($"Full documentation, commands and usage can be found here:\n{docUrl}")
